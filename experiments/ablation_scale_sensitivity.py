"""
Pillar 2: Multi-Scale Receptive Field Sensitivity Ablation
=========================================================
Proves why multi-scale (w in {4, 8, 16, 32}) is necessary vs single-scale baselines.
"""
...
def run_scale_ablation(ticker: str = "SPY") -> pd.DataFrame:
    print(f"\n{'='*70}\n  PILLAR 2: MULTI-SCALE SENSITIVITY ABLATION ({ticker})\n{'='*70}")
    df = prepare_benchmark_dataset(ticker)
    returns = df['Return'].values
    
    scale_configs = {
        'SingleScale_w4': ([4], [1]),
        'SingleScale_w16': ([16], [1]),
        'SingleScale_w32': ([32], [1]),
        'Full_MultiScale_MSOPT': ([4, 8, 16, 32], [1, 2, 4])
    }
    
    ablation_results = []
    
    for name, (ws, ds) in scale_configs.items():
        tokenizer = MSOPTTokenizer(window_sizes=ws, dilations=ds, stride=1)
        ...
        ablation_results.append({
            'Ticker': ticker,
            'Scale_Configuration': name,
            'N_Pattern_Tokens': len(freq_cols),
            'OOS_Accuracy': acc,
            'Sharpe_Ratio': sharpe,
    ...
    print(f"\n--- Scale Sensitivity Ablation Results ({ticker}) ---")
    print(res_df.round(4).to_string(index=False))
    res_df.to_csv(os.path.join(RESULTS_DIR, f"ablation_scale_{ticker.lower()}.csv"), index=False)
    return res_df

if __name__ == "__main__":
    for ticker in ["SPY", "QQQ"]:
        run_scale_ablation(ticker)
